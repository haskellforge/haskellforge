...
def create_predict_fn(checkpoint_path_or_url: str) -> Callable[[str], str]:
    tokenizer = GPT2Tokenizer.from_pretrained("AISE-TUDelft/CodeGPT-Multilingual", do_lower_case=False, sep_token='<EOL>',
    ...
    def codegpt_predict(left_context: str) -> str:
        ...
        with torch.no_grad():
            ...
            for i in range(inputs.shape[0]):
                ...
                pred = [torch.cat([x.view(-1) for x in p] + [zero] * (predict_size - len(p))).view(1, -1) for p in pred]
