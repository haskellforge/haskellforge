...
def train(args, train_dataset, model, tokenizer, fh, pool):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            current_epoch_step = global_step - idx * len(train_dataloader)
            should_skip_step = step < current_epoch_step
            # make sure to skip some steps to match the global step. this is needed when resuming training
            if should_skip_step:
                continue
